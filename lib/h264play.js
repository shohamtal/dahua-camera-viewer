...
export function playRecording(conn, channel, start, end, canvas, { fps = 20, onStatus = () => {}, onProgress = () => {} } = {}) {
  const ctrl = new AbortController();
  const ctx = canvas.getContext('2d');
  let decoder = null, stopped = false, paused = false, firstDrawn = false, sps = null, pps = null, started = false;
  const queue = []; // {key, data(AVCC), ts}
  let fed = 0, drawn = 0, lastReport = -1;
  ...
  function ensureDecoder() {
    ...
    decoder = new VideoDecoder({
      output: (frame) => {
        ...
        frame.close();
        drawn++;
        const sec = Math.floor(drawn / fps);
        if (sec !== lastReport) { lastReport = sec; onProgress(sec); }
  ...
  const pump = setInterval(() => {
    if (stopped || paused || !decoder || decoder.decodeQueueSize > 4) return;
    const c = queue.shift();
    if (!c) return;
